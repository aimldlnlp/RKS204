...
void displaySensorValues_pakdhe(int s1, int s2, int s3, int s4, int s5, int s6, int s7, int s8);
...
void loop() {
  ...
  if (sensor1 > blackThreshold && sensor2 > blackThreshold && sensor6 < whiteThreshold) {
    ...
    while (true) { 
      ...
      controlMotorSpeed_pakdhe(0, 200);
      if (sensor4 > blackThreshold || sensor5 > blackThreshold) {
        break;
      }
    }
  ...
  if (sensor1 < whiteThreshold && sensor2 < whiteThreshold && sensor6 > blackThreshold) {
    ...
    while (true) {
      ...
      controlMotorSpeed_pakdhe(200, 0);
      if (sensor4 > blackThreshold || sensor5 > blackThreshold) {
        break;
      }
    }
...
void readSensors_pakdhe() {
  ...
  // displaySensorValues_pakdhe(sensor1, sensor2, sensor3, sensor4, sensor5, sensor6, sensor7, sensor8);
}
